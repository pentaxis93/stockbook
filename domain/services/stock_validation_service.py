...
class StockValidationService:
    ...
    def validate_price_reasonableness(
        self, stock: StockEntity, current_price: Money
    ) -> None:
    ...
    def validate_grade_price_consistency(
        self, stock: StockEntity, current_price: Optional[Money] = None
    ) -> None:
    ...
    def validate_industry_classification(self, stock: StockEntity) -> None:
        ...
        if not stock.industry_group:
            raise ValidationError(
                "Industry classification is required", field="industry_group"
            )
    ...
    def validate_penny_stock_rules(
        self, stock: StockEntity, current_price: Optional[Money] = None
    ) -> None:
    ...
    def is_blue_chip_candidate(
        self, stock: StockEntity, current_price: Optional[Money] = None
    ) -> BluechipEligibilityResult:
    ...
    def validate_data_completeness(self, stock: StockEntity) -> None:
        ...
        if not stock.name or stock.name.strip() == "":
            raise ValidationError(
                "Stock name is required and cannot be empty", field="name"
            )
        ...
        if not stock.grade or stock.grade.strip() == "":
            raise ValidationError("Stock grade is required", field="grade")
    ...
    def add_custom_rule(
        self, rule_name: str, rule_function: Callable[[StockEntity], bool]
    ) -> None:
