...
class RiskAssessmentService:
    ...
    def assess_portfolio_risk(
        ...
        risk_warnings = self._generate_risk_warnings(
            portfolio_metrics, concentration_risks
        )
        ...
        if beta_risk in [RiskLevel.HIGH, RiskLevel.VERY_HIGH]:
            factors.append(
                RiskFactor(
                    ...
                    mitigation_suggestions=[
                        "Diversify with low-beta stocks",
    ...
    def _identify_concentration_risks(
        ...
        position_allocations = calc_service.calculate_position_allocations(
            portfolio, prices
        )
        concentration_risks = []
    ...
    def _assess_portfolio_sector_risks(
        self, portfolio: List[Tuple[StockEntity, Quantity]], prices: Dict[str, Money]
    ) -> Dict[str, RiskLevel]:
        ...
        industry_allocation = calc_service.calculate_industry_allocations(
            portfolio, prices
        )
        sector_risks = {}
    ...
    def _generate_risk_warnings(
        ...
        if portfolio_metrics.overall_risk_level in [
            RiskLevel.HIGH,
            RiskLevel.VERY_HIGH,
        ]:
            warnings.append(
                "Portfolio risk level is elevated - consider risk management strategies"
            )
        ...
        if critical_concentrations:
            warnings.append(
                f"Critical concentration risk detected in {len(critical_concentrations)} positions"
            )
