...
from .value_objects.portfolio_metrics import (
    PortfolioMetrics, PortfolioAllocation, PositionAllocation, PortfolioSummary
)
...
class PortfolioCalculationService:
    ...
    def calculate_total_value(self, portfolio: List[Tuple[StockEntity, Quantity]], prices: Dict[str, Money]) -> Money:
    ...
    def calculate_position_value(self, stock: StockEntity, quantity: Quantity, current_price: Money) -> Money:
        """Calculate individual position value."""
        return Money(
            current_price.amount * Decimal(str(quantity.value)),
            current_price.currency
        )
    
    def calculate_position_allocations(self, portfolio: List[Tuple[StockEntity, Quantity]], prices: Dict[str, Money]) -> List[PositionAllocation]:
    ...
    def calculate_industry_allocations(self, portfolio: List[Tuple[StockEntity, Quantity]], prices: Dict[str, Money]) -> PortfolioAllocation:
        ...
        for industry, value in industry_values.items():
            percentage = (value / total_value.amount) * Decimal('100') if total_value.amount > 0 else Decimal('0')
            industry_percentages[industry] = percentage
        
        return PortfolioAllocation(industry_percentages, total_value)
    
    def calculate_grade_allocations(self, portfolio: List[Tuple[StockEntity, Quantity]], prices: Dict[str, Money]) -> PortfolioAllocation:
    ...
    def calculate_diversity_score(self, portfolio: List[Tuple[StockEntity, Quantity]], prices: Dict[str, Money]) -> Decimal:
    ...
    def calculate_weighted_average_grade(self, portfolio: List[Tuple[StockEntity, Quantity]], prices: Dict[str, Money]) -> str:
