...
try:
    from error_handling import (
        StockBookError, ValidationError, DatabaseError, BusinessLogicError,
        ErrorLogger, ErrorMessageMapper, StreamlitErrorBoundary,
        MessageSystem, ErrorRecovery
    )
except ImportError:
...
class TestErrorMessageMapper:
    ...
    def test_map_validation_error_message(self):
        ...
        assert "valid" in user_message.lower() or "invalid" in user_message.lower() or "error" in user_message.lower()
        assert user_message != error.message  # Should be different from technical message
    ...
    def test_map_database_error_message(self):
        ...
        assert "already exists" in user_message.lower() or "duplicate" in user_message.lower()
...
class TestStreamlitErrorBoundary:
    ...
    def test_error_boundary_handles_stockbook_error(self, mock_st_error):
        ...
        assert "valid" in call_args.lower() or "invalid" in call_args.lower() or "error" in call_args.lower()
...
class TestErrorRecovery:
    ...
    def test_recovery_suggestions_for_database_error(self):
        ...
        assert isinstance(suggestions, list)
        assert len(suggestions) > 0
        assert any("try again" in suggestion.lower() or "retry" in suggestion.lower() 
    ...
    def test_recovery_suggestions_for_business_logic_error(self):
        ...
        assert isinstance(suggestions, list)
        assert len(suggestions) > 0
        assert any("reduce" in suggestion.lower() or "lower" in suggestion.lower()
                  for suggestion in suggestions)
    ...
    def test_no_suggestions_for_unknown_error(self):
        ...
        if suggestions:
            assert any("contact" in suggestion.lower() or "support" in suggestion.lower()
                      for suggestion in suggestions)
    ...
    def test_actionable_suggestions(self):
        ...
        recovery = ErrorRecovery()
        error = ValidationError("Symbol too long", field="symbol", value="VERYLONGSYMBOL")
