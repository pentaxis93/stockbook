...
class TestDomainEventSerialization:
    ...
    def test_round_trip_serialization(self):
        ...
        original_event = DomainEvent(
            timestamp=datetime(2023, 1, 1, 12, 0, 0), event_id="test-event-123"
        )
    ...
    def test_event_hierarchy_serialization(self):
        ...
        class ProductUpdatedEvent(DomainEvent):
            ...
            def to_dict(self) -> dict:
                ...
                base_dict.update(
                    {"product_id": self.product_id, "changes": self.changes}
                )
                return base_dict

        event = ProductUpdatedEvent(
            product_id="product-123", changes={"price": 99.99, "stock": 10}
        )
