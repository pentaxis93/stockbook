...
class Config:
    ...
    def _load_configuration(self):
        ...
        self.db_path = Path(self._get_env_str('STOCKBOOK_DB_PATH', 'database/stockbook.db'))
        self.schema_path = Path(self._get_env_str('STOCKBOOK_SCHEMA_PATH', 'database/schema.sql'))
        self.test_db_path = Path(self._get_env_str('STOCKBOOK_TEST_DB_PATH', 'database/test_stockbook.db'))
        
        # Database connection settings
        self.db_connection_timeout = self._get_env_int('STOCKBOOK_DB_TIMEOUT', 30)
        self.db_foreign_keys_enabled = self._get_env_bool('STOCKBOOK_DB_FOREIGN_KEYS', True)
        ...
        self.datetime_format = self._get_env_str('STOCKBOOK_DATETIME_FORMAT', '%Y-%m-%d %H:%M:%S')
    ...
    def _setup_validation_rules(self):
        ...
        self.stock_symbol_max_length = self._get_env_int('STOCKBOOK_SYMBOL_MAX_LENGTH', 10)
        
        # Valid stock grades
        self.valid_grades = self._get_env_list('STOCKBOOK_VALID_GRADES', ['A', 'B', 'C'])
        
        # Transaction validation
        self.valid_transaction_types = self._get_env_list('STOCKBOOK_TRANSACTION_TYPES', ['buy', 'sell'])
        ...
        self.portfolio_name_max_length = self._get_env_int('STOCKBOOK_PORTFOLIO_NAME_MAX_LENGTH', 100)
        self.max_risk_per_trade_limit = self._get_env_float('STOCKBOOK_MAX_RISK_LIMIT', 10.0)
    ...
    def _setup_portfolio_defaults(self):
        ...
        self.portfolio_defaults = {
            ...
            'max_risk_per_trade': self._get_env_float('STOCKBOOK_MAX_RISK_PER_TRADE', 2.0),
    ...
    def _setup_feature_flags(self):
        ...
        self.features = {
            ...
            'portfolio_management': self._get_env_bool('STOCKBOOK_FEATURE_PORTFOLIOS', True),
            'transaction_recording': self._get_env_bool('STOCKBOOK_FEATURE_TRANSACTIONS', True),
            ...
            'multi_account': self._get_env_bool('STOCKBOOK_FEATURE_MULTI_ACCOUNT', False),
