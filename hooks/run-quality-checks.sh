...
if [ -n "$CORE_FILES" ]; then
    ...
    CORE_DISABLE="too-few-public-methods,too-many-public-methods,too-many-instance-attributes,unnecessary-pass,wrong-import-order,ungrouped-imports,import-outside-toplevel,line-too-long,no-member,too-many-positional-arguments,duplicate-code,raise-missing-from,fixme,protected-access"
    if ! PYTHONPATH=. pylint --disable="$CORE_DISABLE" \
        --allowed-redefined-builtins=id \
        --max-args=12 --max-locals=15 --max-returns=8 --max-branches=12 \
        --max-statements=50 --max-positional-arguments=8 \
...
if [ -n "$TEST_FILES" ]; then
    ...
    TEST_DISABLE="too-few-public-methods,too-many-public-methods,too-many-instance-attributes,wrong-import-order,ungrouped-imports,line-too-long,no-member,redefined-outer-name,attribute-defined-outside-init,duplicate-code,unused-variable,unused-argument,protected-access,singleton-comparison,pointless-statement,unnecessary-pass,broad-exception-caught,comparison-with-itself,unexpected-keyword-arg,unused-import,logging-fstring-interpolation,no-else-return,raise-missing-from,import-outside-toplevel,unnecessary-negation,missing-class-docstring,missing-function-docstring,abstract-class-instantiated,consider-using-with,too-many-arguments,too-many-positional-arguments,fixme,too-many-lines"
