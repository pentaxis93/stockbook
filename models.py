...
class Stock(BaseModel):
    ...
    symbol: str = Field(
        ...,
        min_length=1,
        max_length=config.stock_symbol_max_length,
...
class Portfolio(BaseModel):
    ...
    name: str = Field(
        ...,
        min_length=1,
        max_length=config.portfolio_name_max_length,
        description="Portfolio name",
    )
    max_positions: int = Field(
        config.portfolio_defaults["max_positions"],
        ge=1,
        le=100,
        description="Maximum number of positions",
    )
    max_risk_per_trade: float = Field(
        config.portfolio_defaults["max_risk_per_trade"],
        ge=0.1,
        le=config.max_risk_per_trade_limit,
...
class Transaction(BaseModel):
    ...
    def validate_type(cls, v):
        ...
        if v not in config.valid_transaction_types:
            raise ValueError(
                f"Transaction type must be one of {config.valid_transaction_types}"
            )
        return v
...
class Target(BaseModel):
    ...
    stock_id: int = Field(..., ge=1, description="Stock ID")
    portfolio_id: int = Field(..., ge=1, description="Portfolio ID")
    pivot_price: Decimal = Field(
        ..., gt=0, decimal_places=2, description="Pivot/breakout price"
    )
    failure_price: Decimal = Field(
        ..., gt=0, decimal_places=2, description="Failure/stop price"
    )
    notes: Optional[str] = Field(None, max_length=500, description="Target notes")
    status: Literal["active", "hit", "failed", "cancelled"] = Field(
        "active", description="Target status"
    )
...
class PortfolioBalance(BaseModel):
    ...
    withdrawals: Decimal = Field(
        Decimal("0.00"), ge=0, decimal_places=2, description="Withdrawals amount"
    )
    deposits: Decimal = Field(
        Decimal("0.00"), ge=0, decimal_places=2, description="Deposits amount"
    )
    final_balance: Decimal = Field(..., decimal_places=2, description="Final balance")
    index_change: Optional[Decimal] = Field(
        None, decimal_places=2, description="Benchmark index change %"
    )
...
class JournalEntry(BaseModel):
    ...
    content: str = Field(
        ..., min_length=1, max_length=10000, description="Journal entry content"
    )
    ...
    transaction_id: Optional[int] = Field(
        None, ge=1, description="Related transaction ID"
    )
